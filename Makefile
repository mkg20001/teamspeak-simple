...
ts3server:
	if [ -d teamspeak3-server_linux_amd64 ]; then rm -rf teamspeak3-server_linux_amd64; fi
	if ! [ -f ts$(VER) ]; then wget -q http://dl.4players.de/ts/releases/3.0.$(VER)/teamspeak3-server_linux_amd64-3.0.$(VER).tar.bz2 -O ts$(VER); fi
	tar -xjf ts$(VER)
